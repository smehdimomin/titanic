train_clean <- train %>% select(Survived,Sex,Pclass,Age,Embarked)
test_clean <- test %>% select(PassengerId,Pclass,Sex,Age,Embarked)
...
write.csv(prediction.df,'result_clean_1.1.csv')
